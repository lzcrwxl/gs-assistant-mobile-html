  ...
  <div id="resumePending">
    ...
    <div class="resumeCtn">
      <DetailTemplate :jItems="jItems">
          <span class="grapProgress" v-show="status==1">
              待处理
          </span>
        <span class="grapProgress" v-show="status==2">
              待面试
          </span>
        <span class="grapProgress" v-show="status==5">
              已取消
          </span>
        <span class="grapProgress" v-show="status==4">
              已完成
          </span>
        <span class="grapProgress" v-show="status==3">
              已拒绝
          </span>
      </DetailTemplate>
      <!--<div class="auntBox">
      ...
      <AuntBoxTemplate :jItems="jItems" :staffInfo="staffInfo" :match="match" :orderNo="orderNo" :class="{'pendingBox': status==1}" :resumeId="resumeId"></AuntBoxTemplate>
      <SourceTemplate :sourceInfo="sourceInfo" v-if="status==2||status==4||status==5"></SourceTemplate>
      <div class="cancelBox" v-show="status==3||status==5">
        <h6>取消理由</h6>
        <p>{{sourceInfo.remarks}}</p>
      </div>
      <div class="auntFooter btn-wrapper" v-show="status==1">
      ...
      <AuntFooter :sourceInfo="sourceInfo" v-if="status==2||status==4||status==3||status==5"></AuntFooter>
    </div>
  </div>
</template>
