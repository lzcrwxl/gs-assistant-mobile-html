  ...
  <div id="resumeBox">
    ...
    <div v-if="$route.meta.isResumeLists">
      ...
      <section>
        ...
        <ul class="aunt-lists">
          <router-link class="aunt-item"
                       :to="{name:'rmDetails',params:{resumeId:item.id,staffId:item.resumeTenantStaffId}}" tag="li"
                       v-for="(item,index) in resumeList" :key="index" v-show="!noStaff">
            <img :src="item.staffInfo.headImage" alt="" class="photo">
            <div class="dib aunt-info">
              <h4 class="mt10">{{item.staffInfo.staffName}}<span
                class="ml30 f26">{{item.staffInfo.serviceTypeValue}}</span></h4>
              <span class="auntStatus" :class="{'pending':item.status==1}">
              {{item.status|status}}
            </span>
              <p class="aunt-detail"><span>{{item.staffInfo.age}}岁 </span>|
                <span>属{{item.staffInfo.zodiacValue}} </span>| <span>{{item.staffInfo.nativePlaceValue}}人 </span>|
                <span>{{item.staffInfo.constellationValue}} </span>| <span>{{item.staffInfo.educationValue}}</span>
              </p>
              <div class="aunt-price"><em>￥</em>{{item.staffInfo.price}} <span class="unit f20">{{item.staffInfo.unitValue}} </span>
              </div>
            </div>
          </router-link>
